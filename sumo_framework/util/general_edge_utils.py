...
from sumo_framework.environment.edge import GeneralizedEdge
...
def get_generalized_edge_to_tls(
    ...
    return GeneralizedEdge(frozenset(head), edges, lanes_offset, length, tail=edges[-1])
...
def get_generalized_edge_from_tls(
    ...
    return GeneralizedEdge(frozenset([edge.getID()]), edges, lanes_offset, length, tail)


def make_generilzed_edges(
    net,
    edge_length_bound: float,
    max_length_for_unite: float,
):
