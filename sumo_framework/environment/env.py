...
from .traffic_controller import (
    TrafficLightsFixedCycleController,
...
class SumoEnvironment(gym.Env):
    ...
    Args:
        ...
        mode (str): ["switch"|"choose_next"|"phases_duration"]
    """
    ...
    def reset(self, seed: Optional[int] = None, **kwargs):
        ...
        if self.mode == "phases_duration":
            self.tls_controllers = {
                ts: TrafficLightsFixedCycleController(
        ...
        self.state_observer = StateObserver(
            self.net,
            {tc_id: tc.phases for tc_id, tc in self.tls_controllers.items()},
        )
        self.departures = {}
        self.arrived = set()
    ...
    def _get_system_info(self):
        ...
        return {
            ...
            "system_average_timeloss": 0.0 if len(self.arrived) == 0 else self.total_timeloss / len(self.arrived),
            "system_mean_speed": 0.0 if len(speeds) == 0 else np.mean(speeds),
            "system_total_travel_time": self.total_travel_time,
            "system_average_travel_time": 0.0 if len(self.arrived) == 0 else self.total_travel_time / len(self.arrived),
        }
